...
np.random.seed(1)
true_beta = np.random.uniform(-0.5, 0.5, p)

# 生成具有自相关性的随机效应
np.random.seed(n)
trend = np.linspace(0, 10, n)
random_effects = trend + np.random.normal(0, 1, n)
# 用于存储预测结果的列表
y_real = []
y_REdeepAR = []
y_deepARestimates = []
...
MCS_result.to_csv("unstationaryMCSpercent.csv")
...
Pvalues_result.to_csv("unstationaryMCSpvalues.csv")
...
plt.legend()
plt.tight_layout()
plt.savefig("unstationaryARIMApredictsimulation.jpg")
plt.show()
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig("unstationaryRandomEffectpredictsimulation.jpg")
plt.show()
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig("unstationaryDeepARpredictsimulation.jpg")
plt.show()
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig("unstationaryRandomEffectDeepARpredictsimulation.jpg")
plt.show()
plt.close()

plt.figure(figsize=(15, 6))
plt.plot(y_real_mean, label="True y", color='blue')
plt.plot(y_REdeepAR_mean, label="REdeepAR Estimated y", linestyle='--', color='red')
...
plt.legend()
plt.tight_layout()
plt.savefig("unstationaryComparethreemethodspredictsimulation.jpg")
plt.show()
plt.close()
...
alpha_result = pd.DataFrame(est_mean, columns=["R2", "RMSE", "COS"])
alpha_result.index = ["RE", "deepAR", "ARIMA", "REdeepAR"]
alpha_result.to_csv("./unstationarypredictioninstructuremean.csv")
