...
def generate_nonstationary_series(n, trend_slope, amplitude, frequency):
    """
    生成一个无论如何差分仍然非平稳的时间序列
    参数:
    - n: 序列长度
    - trend_slope: 趋势的斜率
    - amplitude: 周期成分的振幅
    ...
    # 添加周期成分
    periodic_component = amplitude * np.sin(frequency * np.arange(n))

    # 生成非平稳时间序列
    nonstationary_series = random_walk + trend + periodic_component
...
np.random.seed(1)
true_beta = np.random.uniform(-1, 1, 4)

# 生成具有自相关性的随机效应
np.random.seed(n)
trend_slope = 0.8     # 趋势的斜率
amplitude = 2         # 周期成分的振幅
frequency = 0.5       # 周期成分的频率

# 生成非平稳时间序列
random_effects = generate_nonstationary_series(n, trend_slope, amplitude, frequency)
...
for j in range(num_simulations):
    ...
    cos_RE = np.dot(y_test.to_numpy(), y_RE) / (np.linalg.norm(y_test.to_numpy()) * np.linalg.norm(y_RE))
    ...
    r2_deepAR = r2_score(y_test.to_numpy(), y_forecasts)
    rmse_deepAR = np.sqrt(mean_squared_error(y_test.to_numpy(), y_forecasts))
    cos_deepAR = np.dot(y_test.to_numpy(), y_forecasts) / (np.linalg.norm(y_test.to_numpy()) * np.linalg.norm(y_forecasts))
    ...
    r2_res = r2_score(y_test.to_numpy(), y_REforecasts)
    rmse_res = np.sqrt(mean_squared_error(y_test, y_REforecasts))
    R2_REdeepAR.append(r2_res)
    RMSE_REdeepAR.append(rmse_res)
    cos_REdeepAR = np.dot(y_test.to_numpy(), y_REforecasts) / (np.linalg.norm(y_test.to_numpy()) * np.linalg.norm(y_REforecasts))
...
MCS_result.to_csv("randomwalkMCSpercent.csv")
...
Pvalues_result.to_csv("randomwalkMCSpvalues.csv")
...
plt.legend()
plt.tight_layout()
plt.savefig("randomwalkARIMApredictsimulation.jpg")
plt.show()
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig("randomwalkRandomEffectpredictsimulation.jpg")
plt.show()
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig("randomwalkDeepARpredictsimulation.jpg")
plt.show()
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig("randomwalkRandomEffectDeepARpredictsimulation.jpg")
plt.show()
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig("randomwalkComparethreemethodspredictsimulation.jpg")
plt.show()
plt.close()
...
alpha_result.to_csv("./randomwalkpredictioninstructuremean.csv")
