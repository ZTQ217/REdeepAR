...
def generate_arima_series(phi, theta, d, n):
    """
    根据ARIMA模型公式生成非平稳时间序列
    参数:
    - phi: 自回归参数列表
    - theta: 移动平均参数列表
    - d: 差分次数
    ...
    # 差分次数
    for _ in range(d):
        epsilon = np.cumsum(epsilon)

    # 生成序列
    for t in range(max(len(phi), len(theta)), n):
        ar_term = sum(phi[i] * series[t - i - 1] for i in range(len(phi)))
        ma_term = sum(theta[j] * epsilon[t - j - 1] for j in range(len(theta)))
        series[t] = ar_term + ma_term + epsilon[t]
...
# 生成具有自相关性的随机效应
np.random.seed(n)
random_effects = generate_arima_series(phi, theta, d, n)
...
MCS_result.to_csv("arimaseriesAR1MCSpercent.csv")
...
Pvalues_result.to_csv("arimaseriesAR1MCSpvalues.csv")
...
plt.legend()
plt.tight_layout()
plt.savefig("arimaseriesARIMApredictsimulation.jpg")
plt.show()
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig("arimaseriesRandomEffectpredictsimulation.jpg")
plt.show()
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig("arimaseriesDeepARpredictsimulation.jpg")
plt.show()
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig("arimaseriesRandomEffectDeepARpredictsimulation.jpg")
plt.show()
plt.close()
...
plt.legend()
plt.tight_layout()
plt.savefig("arimaseriesComparethreemethodspredictsimulation.jpg")
plt.show()
plt.close()
...
alpha_result.to_csv("./arimaseriespredictioninstructuremean.csv")
